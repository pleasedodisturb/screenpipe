...
- **agent browser inherits your cookies** — the embedded browser now picks up your existing logged-in sessions from Arc, Chrome, Brave, and Edge on macOS, so the agent can navigate sites you're already signed into without re-auth.
- **markdown meeting note editor** — meeting notes now use an Obsidian-style live markdown editor, with a full transcript side panel and a one-click "copy meeting + transcript" button. see [meeting transcription](/meeting-transcription).
- **cross-device memories sync** — opt-in (Pro) sync of your memories across devices, with each entry tagged by source device. see [AI memory](/ai-memory).
- **PostHog connection** — connect PostHog from **settings → [connections](/connections)**, including a host field for EU and self-hosted instances.
- **Vercel via OAuth** — the Vercel integration is now a one-click OAuth flow instead of pasting an API token. see [connections](/connections).
- **Obsidian vault auto-discovery** — screenpipe finds your Obsidian vaults automatically, with a redesigned connect/disconnect flow. see [obsidian](/obsidian).
- **connections power Pi** — connected integrations are now wired into Pi's system prompt, so chats and pipes know which apps you've linked without you spelling it out.
- **chat picker in collapsed sidebar** — switch between chats without expanding the sidebar.
- **search your chats from memory search** — the global memory search now includes past chats alongside screen and audio results.
- **hide thinking blocks** — new **settings → display → hide thinking blocks** toggle (on by default) to keep model reasoning out of the chat view.
- **local PII redactor** — a new on-device text redactor (auto-downloaded on first launch) replaces the enclave path when you choose the local backend in the [privacy filter](/privacy-filter), and now catches modern provider API key formats out of the box.

### updates

- **default ChatGPT preset is now gpt-5.5** (was gpt-5.4) — new chats pick up the bump automatically.
- **AI credits extend your daily cost cap 1:1** — buying credits now raises your daily ceiling instead of sitting in a separate bucket.
- **QuickBooks** is now on Production Intuit OAuth, so the connection works against live company data.
- **clear cache** has moved from settings → general into settings → storage, alongside the other disk-management controls.
- **meeting summarize** now updates the meeting title and asks before acting, instead of overwriting silently.
- **Apple Intelligence connection** is hidden on Windows and Linux where it can't run.
- **focused document path** capture (TextEdit, Obsidian, Cursor, VS Code, Windsurf, VSCodium, Trae) now works on Windows and Linux too — previously macOS-only.

### bug fixes

- **settings can no longer be wiped on update** — added a four-layer recovery so a bad update can't lose your configuration.
- **pipes panel** — fixed dropdown lag, the stuck loading skeleton, the offline-mode UX, and the missing feedback in pipe discovery.
- **owned browser** — agent webview no longer floats above other apps, retries navigation after install completes, fixes login cookies being empty for some sites, and stops a runaway poll that was pushing macOS WindowServer to 100% CPU.
- **chat** — editing a previous user message now preserves its display, and the AI model picker's search and close behavior in settings is fixed.
- **audio** — fixed a use-after-free during stream stop, capped stop wait at 3 seconds, and contained a speaker-init ORT panic that could take down audio capture.
- **Discord call detection on Windows** is unbroken (had been silently failing since mid-April).
- **macOS reliability** — caught panics in Objective-C → Rust callbacks so they no longer abort the app, dispatched clipboard reads to the main thread, and dropped the SCK cursor / click-ripple compositing that caused redraw glitches.
- **pipes** — orphaned pid-file cleanup no longer sends SIGTERM to screenpipe itself.
- **calendar permission** — the **Fix** button now resets the stale macOS TCC record before re-requesting, instead of being a silent no-op.
- **Claude Desktop on Windows** — MCP setup now supports MSIX installs and injects the auth key correctly. see [MCP server](/mcp-server).
- **Pi chat** — web search extension loads again, and AI memory writes are surfaced via the API skill.
- **AI gateway** — Gemini tool schemas with nested objects now work, requests cascade past 404/403 from individual providers, and Anthropic tool-use IDs are sanitized to the required pattern.
- **macOS DMG** — restored the **Applications** drag-to-install symlink that had gone missing in recent builds.
- **overlay** — phone and app icons no longer expand the pill on hover.
- **meeting notes** — replay-strip caption height is locked so it stops jumping during playback.
- **database** — write-pool timeout under heavy load is fixed, and the original DB error is now surfaced instead of being masked as a pool timeout.
