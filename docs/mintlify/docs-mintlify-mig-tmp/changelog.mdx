...
- **chat source citation footer** — chat replies now show the sources behind each answer (screen captures, database hits, web pages, files, memories, pipes, or commands) in a collapsible footer. expand it to see every source, click through to the originals, and verify what the agent based its answer on.
- **Windows microphone echo cancellation** — Windows users can now enable WASAPI AEC for supported input devices to suppress speaker echo in recordings. toggle it in **settings → recording → microphone echo cancellation**.

### updates

- **faster chat load** — heavy renderers (mermaid diagrams, code blocks) in chat messages now defer their initialization, so opening a long conversation is noticeably quicker.
- **polished live meeting transcript UI** — the in-progress note view and transcript panel have been tightened up for smoother updates and a cleaner layout. see [meeting transcription](/meeting-transcription).
