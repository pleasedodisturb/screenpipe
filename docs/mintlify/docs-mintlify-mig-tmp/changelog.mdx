...
- **drag-to-merge meetings** — drag one meeting onto another in the meetings list to merge them. titles, attendees, and notes from both sides are preserved. see [meeting transcription](/meeting-transcription).
- **smart connection suggestions in chat** — the chat dashboard now surfaces relevant integrations to connect based on what you're asking about, so the agent can reach the right app without manual setup. see [connections](/connections).
- **filter and sort the Connections list** — **settings → [connections](/connections)** has new filter and sort controls to make a long list of integrations easier to navigate.
- **Google OAuth unverified-app walkthrough** — connecting Google now walks you through the "Google hasn't verified this app" screen step-by-step, so the first-time OAuth flow stops dead-ending. see [connections](/connections).
- **resizable left sidebar** — drag the hairline handle on the sidebar edge to set your preferred width across chat, meetings, and the timeline.
- **toggle the meeting live-note notification** — turn off the banner that appears when a live meeting note opens, from **settings → notifications**.
- **toggle auto-append of typed text to meeting notes** — opt out of the behavior that pastes what you type into the active meeting note. see [meeting transcription](/meeting-transcription).
- **collapsible transcript search + caret-follow in the note editor** — collapse the transcript search bar to reclaim vertical space, and the editor now scrolls to keep the caret in view while you type long notes.
- **Windows: agent browser session reuse** — the embedded agent browser on Windows now reuses your existing browser session so signed-in sites work without re-authenticating, with a smarter extension pairing popup. see [connections](/connections).
- **CLI per-parameter overrides for event-driven capture** — fine-tune individual event-driven capture intervals (idle, focus change, app switch, etc.) without rewriting the full config. see the [CLI reference](/cli-reference).

### updates

- **Pro users default to Screenpipe Cloud transcription** — new Pro sign-ins now start on Screenpipe Cloud (Deepgram) instead of local Whisper, with a one-time migration that respects any engine you've already explicitly chosen. see [meeting transcription](/meeting-transcription).
- **meeting-detected notification is a single action** — the in-app meeting-detected toast collapses to one clear primary action instead of competing buttons.
- **OpenAI realtime removed from the live engine dropdown** — the direct OpenAI Realtime option is gone from the live transcription engine list; use Screenpipe Cloud or a supported local engine instead. see [meeting transcription](/meeting-transcription).

### bug fixes

- **live transcript for auto-detected meetings** — meetings that screenpipe auto-detects (no calendar event) now get a live transcript like manually started ones, with workspace-swipe markdown stability and stall notifications when the stream stops. see [meeting transcription](/meeting-transcription).
- **Google Calendar event deduplication across accounts** — events shared between multiple connected Google accounts no longer appear twice in the meetings list, and the nearby-event join CTA has a softer, less interruptive treatment.
- **chat no longer blanks after "summarize with AI"** — clicking summarize from a meeting note opens the chat with content visible instead of an empty page.
- **speaker deletion no longer fails silently** — deleting a speaker now succeeds even when the underlying audio chunk files are missing.
- **running-pipe spinner hides with the sidebar** — collapsing the left sidebar no longer leaves a stray pipe-running spinner floating over the UI.
- **chat empty state hides during pipe streaming** — the "start chatting" prompt no longer flashes underneath a pipe response while it's streaming.
- **focus monitor identity matching** — the focused-window tracker now matches application identity correctly, so app-switch events attribute to the right app.
- **tray shows Error on permission denial** — when screen or audio permissions are missing, the menu-bar/tray status flips to Error instead of staying on "Recording".
- **macOS uses the bundled ffmpeg** — screenpipe now prefers its bundled ffmpeg binary over any version installed on your PATH, so Homebrew/system ffmpeg upgrades can't break capture.
- **audio transcription writes no longer collide** — replacing existing audio transcriptions uses a safe upsert, removing rare constraint errors during reconciliation.
- **Windows CLI postinstall + `npx` collision fixed** — `npm i -g @screenpipe/cli` and `npx @screenpipe/cli …` install and run cleanly on Windows again, with the dead JS wrapper that caused a bin-name collision removed. see the [CLI reference](/cli-reference).
- **Pro cloud audio migration respects your choice** — the automatic Pro → Screenpipe Cloud transcription migration leaves explicit engine selections alone and resets cleanly on logout, so signing out and back in doesn't silently re-route your audio.
- **AI gateway retries cleanly on provider hiccups** — refusal blocks are stripped, sampling 400s retry, and orphaned tool messages are dropped, so transient provider errors don't blow up a chat turn.
- **fallback icon on chat auto-suggestions** — chat suggestion chips show a sparkle icon when the integration-specific icon hasn't loaded, instead of an empty square.
- **Deepgram batching aligned with worker timeout** — Deepgram batch size and the transcription worker timeout now agree, so long meetings stop dropping the tail of the audio.
- **typing fixed in the Connections search input** — the search/filter input on **settings → [connections](/connections)** accepts keystrokes again.
- **on-device PII redactors authenticate with your cloud token** — the Tinfoil text and image redactors now use your signed-in cloud credential automatically, so AI PII removal works without setting an environment variable. see [privacy filter](/privacy-filter).
